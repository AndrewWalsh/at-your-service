...
test("the order of elements in the first argument does not affect the output", async () => {
  ...
  expect(await samplesToJSONSchema([s1, s2])).toEqual(await samplesToJSONSchema([s2, s1]));
});
...
test("equivalent data types have the same structure", async () => {
  ...
  expect(await samplesToJSONSchema([s1, s2])).toEqual(await samplesToJSONSchema([s1, s1]));
});
